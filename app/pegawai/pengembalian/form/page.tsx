...
export default function PengembalianForm() {
    ...
    const handleBatchReturn = async () => {
        ...
        if (!confirm(`Ajukan pengembalian untuk ${selectedIds.length} peminjaman?`)) return;
    ...
    const filteredPeminjaman = peminjaman.filter(item => {
        ...
        return items.includes(searchQuery.toLowerCase()) ||
    ...
    return (
        <div className="min-h-screen bg-[#f5f7fb] w-full">
            <main className="flex-1 flex flex-col">
                ...
                <div className="p-8">
                    <h1 className="text-3xl font-bold mb-2 text-gray-800">Ajukan Pengembalian</h1>
                    <p className="text-gray-500 mb-8">Pilih barang yang ingin Anda kembalikan</p>
                    ...
                    <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 max-w-5xl">
                        <div className="p-6 border-b border-gray-50 flex items-center justify-between bg-white/50 backdrop-blur-sm sticky top-0 z-10">
                            <div className="flex items-center gap-4">
                                <h2 className="text-xl font-bold text-gray-800">Barang yang Dipinjam</h2>
                        ...
                        {filteredPeminjaman.length === 0 ? (
                            <div className="p-20 text-center">
                                ...
                                <h3 className="text-lg font-semibold text-gray-800 mb-1">Tidak Ada Pinjaman</h3>
                                <p className="text-gray-500">Anda tidak memiliki barang yang perlu dikembalikan saat ini.</p>
                            </div>
                        ) : (
                                ...
                                <table className="w-full">
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {filteredPeminjaman.map((item) => {
                                            ...
                                            const isSelected = selectedIds.includes(item.id_peminjaman);
                                                            ...
                                                            {item.detail_peminjaman.map((detail) => {
                                                                ...
                                                                return (
                                                                    <div key={detail.id} className="flex items-center gap-3">
                                                                        <div className={`w-1.5 h-1.5 rounded-full ${overdue ? 'bg-red-400' : 'bg-blue-400'}`}></div>
                                                                        <span className="font-semibold text-gray-800">{inv?.nama}</span>
                        ...
                        <div className="p-8 bg-gray-50/50 border-t border-gray-100 flex justify-between items-center">
                            <div className="text-sm text-gray-500">
                                {selectedIds.length > 0 ? (
                                    <span>Mengembalikan <strong>{selectedIds.length}</strong> peminjaman</span>
                                ) : (
                            ...
                            <div className="flex gap-3">
                                ...
                                >
                                    {submitting ? <LoadingSpinner size="sm" /> : <RotateCcw size={18} />}
                                    Ajukan Pengembalian
                                </Button>
                            </div>
                        </div>
