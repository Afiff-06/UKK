...
export default function Peminjaman() {
        ...
        try {
            ...
            const riwayat = ((data || []) as RiwayatPeminjamanRow[]).map((item) => ({
                ...
                detail_peminjaman: (item.detail_peminjaman || []).map((detail) => ({
                    ...
                    inventaris: {
                        nama: (detail.inventaris as any)?.nama || (Array.isArray(detail.inventaris) ? detail.inventaris[0]?.nama : "") || "",
                        kode_inventaris: (detail.inventaris as any)?.kode_inventaris || (Array.isArray(detail.inventaris) ? detail.inventaris[0]?.kode_inventaris : 0) || 0,
                    },
                })),
            }));
    ...
    const filteredRiwayat = riwayatPeminjaman.filter(item => {
        const barang = item.detail_peminjaman.map(d => d.inventaris.nama.toLowerCase()).join(" ");
        return barang.includes(searchQuery.toLowerCase()) ||
            ...
            <main className="flex-1 flex flex-col">
                ...
                <div className="p-8">
                    <div className="flex justify-between items-start mb-6">
                        <div>
                            <h1 className="text-3xl font-bold mb-2 text-gray-800">Riwayat Peminjaman</h1>
                    ...
                    <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
                        ...
                        {filteredRiwayat.length === 0 ? (
                            <div className="p-20 text-center">
                                ...
                                <h3 className="text-lg font-semibold text-gray-800 mb-1">Tidak Ada Data</h3>
                                <p className="text-gray-500">Belum ada riwayat peminjaman yang ditemukan.</p>
                            </div>
                        ) : (
                                ...
                                <table className="w-full">
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {filteredRiwayat.map((item) => (
                                            <tr key={item.id_peminjaman} className="group hover:bg-blue-50/30 transition-colors">
...
function SummaryCard({ icon, label, value, color }: { icon: React.ReactNode; label: string; value: number; color: 'blue' | 'yellow' | 'red' }) {
    const bgColors = {
        blue: 'bg-blue-50',
        yellow: 'bg-yellow-50',
        red: 'bg-red-50',
    ...
    return (
        <div className="bg-white rounded-3xl p-6 shadow-lg shadow-gray-100/50 border border-gray-50 flex items-center gap-5 transition-transform hover:scale-[1.02]">
            <div className={`w-14 h-14 ${bgColors[color]} rounded-2xl flex items-center justify-center text-2xl`}>
                {icon}
            </div>
            <div>
